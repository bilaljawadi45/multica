...
/**
 * Step identifiers for the onboarding flow. Kept as a union (not an enum)
 * so it stays tree-shakable and easy to extend.
 *
 * welcome → workspace → runtime → agent → complete.
 ...
 *
 * State lives in local useState for now. When progress becomes
 * server-driven (user.settings.onboarding or equivalent), the `useState`
 * pair is replaced by a hook pair — the step components themselves don't
 * change. That's the whole point of keeping the step API dumb.
 *
 * `onComplete` receives the newly-created workspace (if any) so the shell
 * can navigate the user there. For users who already had a workspace
 * and skipped the create step, `onComplete` is called with `undefined` —
 * the shell should just close the overlay without navigating.
 */
